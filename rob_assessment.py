...
# Intro message
intro_message = {"role": "system", "content": script["Intro"]}
# Prompt main body.
...
def process_pdf_stored_in_cloud(file_dict):
    """
    Process all the pdf stored in the cloud. Loop through the dictionary.
    Input: {file_name: file_id} dictionary.
    Output: assessment_explanation, assessment_summary
    """
    ...
    for i, file_name in enumerate(file_dict.keys()):
        print(f"Processing: File {1}/{pdfs_count}. Filename: {file_name}")
...
def call_openai_response_api(messages):
    """
